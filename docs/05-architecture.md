...
│   │   │   ├── flows.ts               ← λ из клиентских блоков, смесь гео-зон
│   │   │   ├── solver.ts              ← [2] распространение λ, ретраи, поглощение
│   │   │   ├── queueing.ts            ← [4] Сакасэгава + Аллен–Каннин, p_fail
│   │   │   ├── replication.ts         ← доля чтений с реплик, липкость, лаг, ожидание догона
│   │   │   ├── cacheModel.ts          ← [6] Ципф, hit ratio, горячий ключ
│   │   │   ├── latency.ts             ← [5] Monte-Carlo по дереву вызовов
│   │   │   ├── consistency.ts         ← [5а] аномалии A1–A8, расхождение при разрыве
│   │   │   ├── contention.ts          ← [5а] конкуренция за ключ: повторы и сериализация
│   │   │   ├── derived.ts             ← [6] хранилище, логи, egress, ключи идемпотентности
│   │   │   ├── cost.ts                ← [6б] стоимость по профилям цен
│   │   │   ├── costDrivers.ts         ← какие параметры двигают счёт и в какую статью
│   │   │   ├── provisioned.ts         ← выделенные IOPS: что оплачивается сверх объёма
...
│   │   │   ├── scenarios.ts           ← 17 сценариев, из них 10 transient
...
│   │   └── common/                    ← Dialog, ErrorBoundary, Icon (SdIcons + UiIcons), ResizeHandle,
│   │                                     Waterfall
...
│   ├── services/                      ← storage, файлы, сериализация, миграции формата схемы,
│   │                                    конструктор схем, воркеры, share-link, экспорт PNG
│   │                                    и Markdown, service worker, referenceBundle
│   ├── styles/                        ← variables.css, index.css
│   └── utils/                         ← format.ts, panelSize.ts, canvasView.ts (свёрнутые группы,
│                                        режимы показа регионов), paramSlider.ts, waterfall.ts,
│                                        nodeName.ts (подпись → роль → тип блока),
│                                        paramSections.ts (группировка параметров),
│                                        popoverPlacement.ts (сторона и запас места карточки),
│                                        timeline.ts (курсор и метрики ряда времени),
...
* **Язык интерфейса по умолчанию — английский**, и в хранилище попадает только **явный** выбор
  игрока: ключ `sd-flow-language-choice` пишет переключатель в шапке, детектор его читает и сам
  ничего не кэширует (`caches: []`). Автокэш `i18next` записывал в старый ключ `sd-flow-language`
  разрешённый язык при первом же открытии, поэтому у всех, кто заходил до 1.6, там навсегда
  оставался русский и новое умолчание до них не доезжало; старый ключ теперь стирается при старте.
  Кто выбрал язык руками — выбор переживает перезагрузку, как и раньше.
* Размер контейнера (`region`, `az`, `vpc`, `k8s-cluster`) живёт в `SchemeNode.size` и берётся
  только у блоков формы `container`: остальные узлы меряет браузер, и запоминать их размер незачем.
  React Flow пишет результат растягивания в `node.width` / `node.height`, а не в `node.style`,
  поэтому единый порядок чтения размера (`width` → `style` → `measured`) задан в
  `utils/nodeSize.ts` — им пользуются сериализация, экспорт в PNG и попадание узла в контейнер.
  Растягивание — правка схемы: обмер узла браузером (`dimensions` без `setAttributes`) правкой
  не считается, а завершённая транзакция растягивания поднимает `revision` и попадает в автосейв.
* Экспорт/импорт — тот же JSON.
* Шаринг — сжатие штатным `CompressionStream('deflate-raw')` в `location.hash`, с деградацией в
  plain там, где API нет; при превышении ~8 КБ предлагается файл.
...
| `uiStore` | панели, тема, язык, зум, открытые окна проб, вид холста |

Вид холста (описания параметров в инспекторе, тепловая карта, миникарта) живёт в `uiStore` и
переживает перезагрузку через `localStorage`. Умолчание миникарты зависит от ширины окна: до 768 px
...
**Что из этого сложилось на практике (1.7).** 1044 теста в 72 файлах, все на Vitest. Ни Testing
Library, ни fast-check, ни Playwright в проект не пришли: property-based проверки заменены
табличными прогонами по каталогу (`capacity-coverage`, `cost-coverage`), а интерфейс проверяется
либо вынесенной в чистый модуль логикой, либо рендером компонента в строку через
`renderToStaticMarkup`. Покрытие не измеряется — пункт NFR-8 в [06-backlog.md](06-backlog.md) открыт.
...
| `Dialog`, `ErrorBoundary`, `Icon` | те же, `DspIcons` → `SdIcons` (99 родовых иконок блоков) плюс `UiIcons` (54 иконки интерфейса); всё инлайновым SVG, без вендорских логотипов и без сторонних шрифтов (ADR-10, ADR-17) | почти как есть |
...
| Панель заданий | `components/panels/Challenges` | Требования в реальном времени, отчёт со звёздами, эталонные решения после сдачи. Звёзды на карточке берутся одной выборкой `earnedProgress` по `refKey` — каталог, три набора практики и свои задания читают их одинаково |
...
| Ход времени | `sim/transient.ts` | Прогон по шагам: лаг автоскейлера, очереди с памятью между шагами, прогрев кэша; ряд `timeline` в `SimResult`, 17 сценариев (10 transient) |
...
| Таймлайн и скраббер (FR-SIM-11) | `panels/Dashboard/Timeline.tsx`, `utils/timeline.ts` | Ряды transient-прогона: нагрузка, утилизация, p99, backlog, ошибки, инстансы; полосы нарушения SLO. Ползунок времени и курсор: по умолчанию курсор встаёт на первое нарушение, ведётся мышью по графику, показывает значение среза и самый горячий узел в этот момент. Логика курсора — чистые функции в `utils/timeline.ts`, поэтому покрыта тестом без DOM |
...
| PWA и офлайн (NFR-6, FR-PLT-4) | `public/manifest.webmanifest`, `public/sw.js`, `services/serviceWorker.ts` | Установка и офлайн-запуск статики; документ берётся из сети с откатом в кеш, файлы с хэшем в имени — из кеша (ADR-16). Иконки интерфейса нарисованы инлайновым SVG, поэтому сторонних домена и шрифта в загрузке нет (ADR-17); ленивые словари `help` и `hints` догружаются на простое после регистрации воркера, чтобы справка работала без сети, даже если её не открывали |
...
| Подсказки к параметрам (FR-PRM-5) | `panels/Inspector`, `hooks/useParamHelp.ts`, `locales/{ru,en}/hints.json` | 671 подсказка на язык: смысл параметра, единица измерения, рабочий диапазон. Единица стоит у поля всегда; текст приходит по наведению на имя параметра, а тумблер «Описания параметров» раскрывает его под каждым полем сразу. Тумблер выключен по умолчанию — иначе панель втрое длиннее — и помнится между сессиями |
...
### 12.5. Что добавила версия 1.4

Версия закрывает пробелы MVP из [06-backlog.md](06-backlog.md) — там же отмечено, что именно
закрыто и что осталось.

| Появилось | Где | Суть |
|---|---|---|
| Потолок схемы (FR-SIM-10) | `sim/ceiling.ts`, `panels/Dashboard` | Бинарный поиск по множителю трафика: удвоение до насыщения плюс 12 делений пополам; ответ — RPS, множитель, узел-ограничитель и его `boundBy`. Считается по кнопке в воркере, отдельным чанком сборки |
| Отмена устаревшего расчёта (FR-SIM-13) | `services/simulationService.ts` | Новый расчёт снимает предыдущий: устаревший промис отклоняется меткой `superseded`, воркер убивается `terminate`, чужие запросы переносятся на свежий воркер |
| Гео-маршрутизация и режимы репликации (FR-SIM-15/16) | `sim/routing.ts`, `sim/flows.ts` | Смесь зон клиента (`geoDistribution` + `geoSpread`) → регион по политике `latency` / `geo` / `failover`, раздельные доли чтений и записей на ребре, `read-local-write-global` уводит записи в `writeRegion` и платит его RTT, `sharded-by-geo` раскладывает ключи по домашним регионам |
| Ожидание зависимостей (FR-SIM-8) | `sim/solver.ts`, `sim/resources.ts`, `sim/queueing.ts` | Слот пула держится `S + S_зав`, ограничитель `blocking` режет ёмкость вызывающего, а Сакасэгава и вероятность таймаута считаются от того же времени удержания; плечо ограничено таймаутом ребра, асинхронные рёбра пул не держат ([02-simulation.md](02-simulation.md) §4.а) |
| Доступность «k из n» и цена failover (FR-SIM-7) | `sim/availability.ts` | Биномиальная группа с кворумом от блока, штраф за время переключения, обход графа с альтернативами за балансировщиками |
| Средства смягчения аномалий (FR-SIM-18) | `sim/consistency.ts`, `sim/replication.ts`, `sim/latency.ts` | Кворум `R + W > N`, липкие чтения, объявленные гарантии сессии и CRDT гасят соответствующие аномалии; у каждого средства своя цена — липкие чтения возвращают нагрузку на primary, `staleReadPolicy = wait-for-lag` платит средним лагом в p99, липкий регион держит сессию в своём регионе |
| Бэкапы и премия за managed (FR-SIM-5/6) | `sim/derived.ts`, `sim/cost.ts` | Объём бэкапов у `sql` и `nosql` по политике §8.5 и плата за него по ставке профиля; там же поток логов (если своего сборщика в схеме нет) и ключи идемпотентности; `managedMultiplier` профиля цен умножает compute, storage и requests |
...
### 12.6. Что добавила версия 1.5

| Появилось | Где | Суть |
|---|---|---|
| Цена конкурентного доступа (FR-SIM-18) | `sim/contention.ts`, `sim/solver.ts` | `optimistic` платит повторами при контенции (они идут в `λ_offered` и поднимают ρ), `pessimistic` — ограничителем ёмкости `key-serialization` = `keys / (writeShare × 2S)`; окно чтения-изменения-записи и число ключей теперь считаются в одном месте для аномалии A4, изоляции и обеих цен |
| Ключи идемпотентности (FR-SIM-18) | `sim/derived.ts`, `sim/constants.ts` | TTL 24 ч и 256 байт на ключ: объём начисляется вызывающему блоку перед провайдером с `idempotencyRequired` и идемпотентному консьюмеру; синхронный ретрай в такой провайдер без ключа даёт аномалию A6 |
| Сценарий `split-brain` (FR-SIM-9) | `sim/scenarios.ts`, `sim/consistency.ts` | Разрыв на 120 с: обе стороны принимают записи, аномалия `divergent-replicas` показывает накопленное расхождение, слияние считается по `conflictResolution`, кворум `R + W > N` расхождение снимает |
| Источник аномалии и цена средств (FR-DSH-9) | `sim/consistency.ts`, `panels/Dashboard` | Строка аномалии раскрывается формулой и подставленными значениями, клик по узлам подводит камеру; ниже — список включённых средств смягчения с тем, что каждое снимает. Список приходит из движка (`ConsistencyResult.mitigations`), поэтому не может разойтись с расчётом |
| Сводка схемы в доке (FR-DSH-1) | `panels/Dashboard` | Число блоков, связей и блоков без модели считается по результату расчёта, а не по холсту |
| Миграции формата схемы (FR-STO-2) | `services/schemeMigrations.ts` | Один вход для любого недоверенного JSON: версия формата, версия модели, недостающие поля по умолчанию, выброшенные незнакомые блоки и осиротевшие связи — с отчётом, который показывается тостом. Схема из будущей версии отвергается с отдельным сообщением |
| Модель согласованности по умолчанию (FR-PLT-7) | `store/uiStore.ts`, `dialogs/SettingsDialog.tsx` | Галочка «запомнить как режим по умолчанию»: значение живёт в настройках приложения и применяется к каждой новой схеме |
| Рубрика до сдачи (FR-CHL-2) | `challenges/rubric.ts`, `panels/Challenges` | `RUBRIC_AXES` и `RUBRIC_WEIGHTS` экспортируются из движка, панель показывает их до отправки решения вместе с ценой подсказок и правок запертых параметров |
| Вклад узлов в вердикт (FR-CHL-5) | `challenges/predicates.ts`, `panels/Challenges` | `RequirementEvaluation.contributions` — до трёх узлов с долей: хопы задержки, простой узлов, загрузка, стоимость |
| Перенос прогресса (FR-CHL-8) | `services/progressTransfer.ts`, `store/challengeStore.ts` | Файл `sd-flow-progress` с обоими наборами; загрузка сливает «лучшее из двух» и чинит битые записи вместо того, чтобы верить им на слово |
| Поповер параметров (FR-PRM-1) | `canvas/ParamPopover.tsx`, `utils/popoverPlacement.ts`, `store/uiStore.ts` | Двойной клик по блоку открывает карточку с первыми шестью параметрами прямо на холсте; закрывается Escape, кликом мимо и повторным двойным кликом по блоку, кнопка внизу открывает инспектор. Ширина живёт в размерах панелей (`panels.popover`), край карточки тянется мышью, двойной клик по краю возвращает умолчание. Масштаб холста погашен обратным `scale`, поэтому на отдалённом виде подписи не мельчают; `placePopover` выбирает сторону от блока и не даёт карточке уползти под инспектор. Платящие параметры помечены знаком `$`, как в инспекторе |
| Слайдеры и пресет инстанса (FR-PRM-3) | `common/ParamInput`, `utils/paramSlider.ts`, `engine/instancePresets.ts` | Один контрол параметра на инспектор и поповер: у числового поля с границами появляется ползунок (логарифмический, когда диапазон шире тысячи крат). Пресет XS–XL масштабирует размерные параметры блока от его же дефолтов, текущий пресет определяется обратно |
| Авто и override производных (FR-PRM-7) | `sim/cacheModel.ts`, `sim/flows.ts`, `panels/Inspector` | `hitRatioMode` + `hitRatioOverride` у блоков с моделью кэша: расчёт или объявленное число, одним решением на показ и на поглощение. У клиента появилась двусторонняя строка «Нагрузка»: правка RPS пересчитывает DAU обратно (`dauForRps`) |
| Сворачивание группы (FR-CNV-6) | `utils/canvasView.ts`, `canvas/GroupNode.tsx` | Представление холста — чистая функция над графом: содержимое свёрнутого контейнера прячется, сам он сжимается до карточки со счётчиком, пересекающие границу связи переводятся на контейнер и складываются в одно ребро-агрегат с числом слитых. Схема и расчёт не меняются: сворачивание живёт в `uiStore`, а не в документе |
| Переключатель регионов (FR-CNV-6b) | `canvas/RegionTabs.tsx`, `store/uiStore.ts` | Три режима поверх того же представления: все сразу, по одному вкладками, все свёрнутые. Панель появляется, когда регионов больше одного |
| Зеркальные регионы (FR-CNV-6a) | `services/mirrorRegions.ts`, `store/graphStore.ts` | `mirrorOf` на регионе (по id или коду) инстанцирует содержимое прототипа с id `<узел>@<регион>`, правки прототипа доезжают до копий, очистка поля отвязывает их, исчезновение прототипа удаляет. Синхронизация идемпотентна и не пишется в историю undo (ADR-13) |


### 12.7. Что осталось на следующие фазы

* Полный перечень нереализованного в модели — [02-simulation.md](02-simulation.md) §15.3;
  рабочий список по требованиям PRD — [06-backlog.md](06-backlog.md).
...
В историю undo/redo пишутся структурные правки, а перетаскивание и растягивание оформляются
транзакцией (`beginTransaction`/`commitTransaction`). Служебные изменения React Flow (`select`,
`dimensions` от обмера) в историю не попадают и схему изменённой не помечают, но завершённая
транзакция растягивания — помечает: иначе новый размер контейнера не попадал бы в автосейв.
...
| ADR-13 | Регионы-зеркала — материализованные узлы с полем `mirrorOf` в данных, а не ссылка, разворачиваемая внутри движка | Первая редакция решения берегла граф от утроения и предлагала разворачивать зеркало только в движке. Так не выходит: всё, что ниже по течению, ключуется по id узла — результаты, пробы, находки, тепловая карта, предикаты приёмки и подвод камеры. Развёрнутые внутри движка узлы получили бы синтетические id, которым на холсте не соответствует ничего, и находка «Postgres перегружен» указывала бы на блок, которого пользователь не видит и не может починить. Поэтому зеркало — настоящие узлы с производными id `<прототип>@<регион>`; синхронизацию делает чистая идемпотентная функция `mirrorGraph` (`services/mirrorRegions.ts`) после каждой правки. В историю undo она не пишется (величина производная), правки прототипа доезжают до копий, а очистка `mirrorOf` не удаляет копии, а отвязывает их. Цена — граф на три региона втрое больше — реальна, но видима; цена альтернативы (невидимые узлы в результатах) хуже |
| ADR-15 | Справка по блоку и подсказки к параметрам — ленивые словари локали, а не поля `ComponentDefinition` | Текстов много: 127 блоков × (описание, ёмкость, практики, ошибки) и 671 параметр, по два языка — 639 КБ на русском и 425 КБ на английском (177 и 146 КБ в gzip). В `initComponents` они попали бы в главный бандл и грузились бы на каждый старт ради экрана, который открывают по кнопке. `locales/{ru,en}/{help,hints}.json` подключаются через `import()` в `referenceBundle.ts` и добавляются в i18next как namespace в рантайме. Ядро остаётся без текстов, а блок и его справка связаны только полем `helpId` — тест `blockReference` требует запись на обоих языках для каждого блока и каждого параметра и запрещает висячие ключи. Ёмкость в справке не хранится текстом, а считается моделью блока на дефолтных параметрах — расходиться с движком ей нечем |
| ADR-17 | Иконки интерфейса — инлайновый SVG в коде, а не шрифт со стороннего домена | 54 иконки приходили шрифтом Material Icons с `fonts.googleapis.com`: сервис-воркер кросс-доменные запросы не кеширует, поэтому без сети на месте иконок оставались слова (`delete_outline`), а первая загрузка ждала блокирующий CSS чужого домена и раскрывала IP игрока Google. Набор перерисован родовыми метафорами в `common/Icons/UiIcons.tsx` тем же контуром, что и 99 иконок блоков в `SdIcons.tsx`; `iconGlyph.ts` ищет имя в обоих наборах, неизвестное имя рисуется заглушкой-блоком, а не текстом. Цена — 54 иконки нужно нарисовать и держать: тест `icons` требует, чтобы каждое имя из разметки было нарисовано, и запрещает висячие иконки в наборе |
...
| ADR-16 | Документ — из сети с откатом в кеш, файлы с хэшем в имени — из кеша, смена версии кеш очищает | Первая редакция сервис-воркера отдавала из кеша всё, включая `index.html`, и обновляла его в фоне. Это значит, что после релиза пользователь видит прошлую сборку: свежий `index.html` доезжает только к следующему открытию, а ссылается он на файл с прежним хэшем, который лежит рядом в том же кеше. Жалоба «кнопка была, а теперь её нет» — ровно этот случай, а не правка кода. Теперь навигация идёт в сеть и падает в кеш только при отказе (офлайн-запуск сохраняется), файлы с хэшем в имени неизменяемы и берутся из кеша, а страница сообщает воркеру свою версию: при расхождении с меткой `build-mark` кеш удаляется целиком и греется заново. Версия видна в шапке, поэтому вопрос «какая сборка открыта» решается взглядом, а не сверкой бандла |
