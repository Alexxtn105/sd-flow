...
| PWA | `public/manifest.webmanifest`, `public/sw.js`, `services/serviceWorker.ts` | Установка и офлайн-запуск статики; документ берётся из сети с откатом в кеш, файлы с хэшем в имени — из кеша (ADR-16) |
...
| Подсказки к параметрам (FR-PRM-5) | `panels/Inspector`, `hooks/useParamHelp.ts`, `locales/{ru,en}/hints.json` | 669 подсказок на язык: смысл параметра, единица измерения, рабочий диапазон. Единица стоит у поля всегда; текст приходит по наведению на имя параметра, а тумблер «Описания параметров» раскрывает его под каждым полем сразу. Тумблер выключен по умолчанию — иначе панель втрое длиннее — и помнится между сессиями |
...
| Гео-маршрутизация и режимы репликации (FR-SIM-15/16) | `sim/routing.ts` | Зона клиента → регион по политике `latency` / `geo` / `failover`, раздельные доли чтений и записей на ребре, `read-local-write-global` уводит записи в `writeRegion` |
| Ожидание зависимостей (FR-SIM-8) | `sim/solver.ts`, `sim/resources.ts` | Слот пула держится `S + S_зав`, ограничитель `blocking` режет ёмкость вызывающего; плечо ограничено таймаутом ребра, асинхронные рёбра пул не держат ([02-simulation.md](02-simulation.md) §4.а) |
...
| Средства смягчения аномалий (FR-SIM-18) | `sim/consistency.ts` | Кворум `R + W > N`, липкие чтения, объявленные гарантии сессии и CRDT гасят соответствующие аномалии |
| Бэкапы и премия за managed (FR-SIM-5/6) | `sim/derived.ts`, `sim/cost.ts` | Объём бэкапов у `sql` и `nosql` по политике §8.5; `managedMultiplier` профиля цен умножает compute, storage и requests |
