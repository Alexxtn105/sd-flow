...
| Поповер параметров (FR-PRM-1) | `canvas/ParamPopover.tsx`, `store/uiStore.ts` | Двойной клик по блоку открывает карточку с первыми шестью параметрами прямо на холсте; закрывается Escape, кликом мимо и повторным двойным кликом, кнопка внизу открывает инспектор |
