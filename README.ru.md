...
* **Findings** — 14 правил движка (перегрузка, насыщение, шторм ретраев, SPOF, горячий ключ,
...
* **Графической части у части проб**: `probe-rps` без тайм-серии, `probe-latency` без гистограммы,
  `probe-heatmap` без оверлея на схеме — эти измерители пока отдают число, а не картинку.
